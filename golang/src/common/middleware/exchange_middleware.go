...
func newExchangeMiddleware(exchangeName string, routingKeys []string, connectionSettings ConnSettings) (Middleware, error) {
...
func (exchange *ExchangeMiddleware) connect(connectionSettings ConnSettings) error {
	url := fmt.Sprintf(
...
func (exchange *ExchangeMiddleware) StartConsuming(callback func(Message, func(), func())) (result error) {
	if callback == nil {
		return ErrMessageMiddlewareMessage
	}
...
func (exchange *ExchangeMiddleware) consumeDeliveries(
	...
	callback func(Message, func(), func()),
) error {
...
func (exchange *ExchangeMiddleware) consumptionStatus(consumption *exchangeConsumption) (bool, error) {
	...
	if exchange.connection.IsClosed() {
		return true, ErrMessageMiddlewareDisconnected
	}
...
func (exchange *ExchangeMiddleware) releaseConsumer(consumption *exchangeConsumption) error {
	...
	defer exchange.lifecycleMutex.Unlock()
	if exchange.connection.IsClosed() {
		releaseError = ErrMessageMiddlewareDisconnected
	}
...
func (exchange *ExchangeMiddleware) Send(message Message) error {
	exchange.lifecycleMutex.Lock()
	defer exchange.lifecycleMutex.Unlock()
